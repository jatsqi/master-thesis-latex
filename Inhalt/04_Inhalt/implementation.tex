...
% 0.5 Seiten
	...
	In the context of this work, the algorithm including all described classifiers, scores and strategies from Chapter \ref{chap:tree} were implemented and integrated into \ac{GCG}, with the only exception being the unordered variant of the \textit{voting classifier} \footnote{In practice, almost all tested models obeyed the precondition of coherent constraint blocks}.
	...
	% 2 Seiten
		...
		\subsubsection{Representing a Tree Node}
			...
			\begin{figure}[ht!]
				\centering
				\includesvg[scale=0.75, inkscapelatex=false]{Bilder/PlantUML/out/tree_node/tree_node}
				\caption{a}
				\label{fig:impl:structures:node}
			\end{figure}

			One of the \textit{central} data structure is the \ac{SRT}, for which a shortened version is shown in Figure \ref{fig:impl:structures:node}.
	...
	% 1 Seite
		...
		\begin{itemize}
			...
			\item A function \lstinline[mathescape]|addIfMissing($\cdot$)| adding the tree node if it doesn't exist already. These two actions are performed \textit{atomically}, i.e., to an outside observer the method behaves as if it executes both operations at once.
		\end{itemize}
	...
	% 1 Seite
					...
					\While{true}
						...
						\For{$cell \gets \mathrm{Cells}_q$}
							\For{$f_{\mathrm{strat}} \in strategies$}
								\If{the pair $(q, strat)$ is forbidden by some rule}
		...
		\label{chap:impl:threading:queue}
			...
			\begin{enumerate}
				\item A variable \lstinline|queueNonEmptyCond|, which blocks workers until the queue is non-empty.
				\item The variable \lstinline|workersWaitingCondVar|, which blocks workers until the queue \textit{is} empty and all workers are being blocked. This variable is being to wait until the correct moment to terminate the search as explained below.
			\end{enumerate}

			Algorithm \ref{algo:impl:queue} requires an addition mechanism which we integrate into the queue data structure itself: a way to send all workers a termination signal. We will only use the signal to notify all threads to shut down until all possible nodes have been expanded, that is, until all $k$ workers have called $\textproc{Pop}$ and were suspended because the queue is empty. If this condition is met, we add $k$ instances of a special element $\Omega$ to the queue, unblocking the workers. As soon as a worker pops an element from which corresponds to $\Omega$, it terminates.
			These special elements are often referred to as \enquote{poison pills} and are used to communicate with workers without modifications to the interface of the data structure. \todo{cite}
			...
			\begin{algorithm}[ht!]
				...
				\begin{algorithmic}
					\Function{push}{queue, element}
						\State \Call{Lock}{mutex} \Comment{Protect queue}
						\State \Call{Push}{queue, element}
						\State \Call{Unlock}{mutex}
						\State \Call{Notify}{queueNonEmptyCond} \Comment{Notify other threads that queue size changed}
					\EndFunction
				\end{algorithmic}
				\caption{text}
				\label{algo:impl:queue:sync:push}
			\end{algorithm}
			...
			\begin{algorithm}[ht!]
				...
				\begin{algorithmic}
					\Function{pop}{queue}
						...
						\Statex
						\State $numWaiting \gets numWaiting + 1$
						\State \Call{Notify}{workersWaitingCondVar}
						\State \Call{WaitFor}{queueNonEmptyCond, mutex}
						\State $numWaiting \gets numWaiting - 1$
						\Statex
						\State $element \gets \Call{Pop}{queue}$
						\State \Call{Unlock}{mutex}
						\State \Return $element$
					\EndFunction
				\end{algorithmic}
				\caption{text}
				\label{algo:impl:queue:sync:pop}
			\end{algorithm}
			...
			\begin{algorithmic}
				\Function{injectPoison}{queue, numWorkers}
					\State \Call{Lock}{mutex}
					\State \Call{WaitFor}{workersWaitingCondVar, mutex}
					\Repeat
						\State \Call{Push}{queue, $\Omega$}
					\Until $numWorkers$ element have been added
					\State \Call{Unlock}{mutex}
				\EndFunction
			\end{algorithmic}
			\caption{text}
			\label{algo:impl:queue:sync:inject}
			\end{algorithm}